 ...
 * @file tensor.hpp
 * @author Aska Lyn
 * @brief A C++ implementation of multi-dimensional tensor data structures for
 * deep learning
 * @details This header defines a templated Tensor class that supports 1D, 2D,
...
public:
   ...
   * @param shapes A vector specifying dimensions [channels, rows, cols] or
   * [rows, cols] or [size]
   * @details Infers the tensor dimensionality from the number of elements
   *          in the shape vector: 1 element = 1D, 2 elements = 2D, 3 elements =
   * 3D.
   */
  ...
  /**
   * @brief Move assignment operator - transfers ownership from a temporary
   * tensor
   * @param tensor The rvalue reference to the source tensor
   ...
   * @param sizes Padding sizes for each dimension [pad_top, pad_bottom,
   * pad_left, pad_right] or similar format depending on implementation
   * @param padding_value The value to fill in padded regions (note: param name
   * in original code suggests it may default to 0)
   ...
   * @details Changes the tensor's dimensions while preserving total element
   * count. The underlying data is reinterpreted according to the new shape.
   * @note The new shape must be compatible (same total size) or reallocation
   * occurs.
   */
   ...
   * @param row_major If true, flattens in row-major order; otherwise,
   * column-major
   ...
   *          Example: Transform([](float x) { return std::max(0.0f, x); }) for
   * ReLU.
   */
   ...
   * @details Returns a pointer to the beginning of the 2D slice at the given
   * channel.
   */
...
private:
  std::vector<uint32_t> raw_shape_; ///< Stores the original shape dimensions
                                    ///< [channels, rows, cols]
